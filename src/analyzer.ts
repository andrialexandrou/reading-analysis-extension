...
export default function (text) {
  ...
  data.paragraphs = text.split("\n").length;
  const score = readingLevel(text);
  if (!score) {
    throw new Error("unable to calculate reading level");
  }
  ...
  return {
    ...
    // output: hardSentences.join(" "),
    adverbs: {
      label: "Adverbs",
      count: data.adverbs,
      ideal: Math.round(data.paragraphs / 3),
    },
    passiveVoice: {
      label: "Passive Voice",
      count: data.passiveVoice,
      ideal: Math.round(data.sentences / 5),
    },
    complex: {
      label: "Complex Phrases",
      count: data.complex,
      ideal: 0,
    },
    hardSentences: {
      label: "Hard Sentences",
      count: data.hardSentences,
      ideal: 0,
    },
    veryHardSentences: {
      label: "Very Hard Sentences",
      count: data.veryHardSentences,
      ideal: 0,
    },
  };
}
